...
This repository versions the Railway wrapper independently from OpenCode. A
new upstream version does not force the wrapper to use the same version number.
...
3. Include the OpenCode version in the release title or notes, such as
   `v2.0.0 — OpenCode 1.18.23`.
...
5. When starting a new major or minor line, update the public Railway
   template's source image to `ghcr.io/joeychilson/railway-opencode:X.Y` only
